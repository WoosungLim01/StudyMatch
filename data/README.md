# StudyMatch — Data Layer

Part of the whole project — see the [repo root README](../README.md) for how this
fits with [`app.py`](../app.py) (the server), [`ui/`](../ui) (the survey + admin
pages), and [`algorithm/`](../algorithm) (the matching math). This folder owns
the schema, the synthetic sample dataset, and the live database.
...
is a real, live database that real people can be added to via the survey; see
...
- **The survey** (`ui/survey.html`, run via [`../app.py`](../app.py) — see the
  [repo root README](../README.md#running-it)) — the normal path once real
  people are actually taking this. Signs them in AND places them in a group
  immediately.
...
Both use [`algorithm/compatibility.py`](../algorithm/compatibility.py) for
scoring — same formula everywhere, so the two paths can't silently drift apart.
...
**refuses to run** once any real student *or any login account* exists — pass
`--force` if you really do want to blow the database away and start over from
pure synthetic data. The only way to remove someone (real *or* fake) is the
admin page's delete button
(`ui/admin.html`).
...
- **`course_chat_messages.json`** ([generator](generate_sample_data.py#L551)) — each
...
- **`group_feedback.json`** ([generator](generate_sample_data.py#L574)) — nothing here
